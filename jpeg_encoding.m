function [All_bitstreams,encoding_time,WPSNR,New_X_hat] = jpeg_encoding(X_float)
...
for loop = 1:image_chanel
    for i=1:image_length
        mean_total(1,i) = mean((X_float(i,:,loop)-New_X_hat(i,:,loop)).^2.*cos((i+0.5-image_length/2)*pi/image_length));
    end
...
%{
mse_jpeg = mean(mean(mean((X_float-New_X_hat).^2)));
